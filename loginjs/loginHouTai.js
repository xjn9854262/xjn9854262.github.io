...
function showGame() {
    $('.goplay').click(function () {
        ...
        if (state == 0 && !whitelist) {//不是白名单
            if ($("#alert").length <= 0) {
                ...
                loadHttp(url, function (data) {
                    $('.alertContent').html("维护时间：<br>" + data);
                });
            }
            showAlert();
            return;
    ...
    version = loginArg.version;
    loginArg.uid = loginArg.account;
    role = loginArg.account;
    console.log(loginArg);
    $(".goplay").trigger("click");
    // if (isTest || DEBUGM) {
    //     initLoginData();
    // } else {
    //     initData();
...
function initLoginData() {
    if (loginArg) {
        var selectD;
        if (window.localStorage.selectDate && DEBUGM && false) {
